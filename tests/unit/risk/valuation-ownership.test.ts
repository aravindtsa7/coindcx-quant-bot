...
function closeContext(pair = openPair()) {
  ...
  return resealContext(makeContext({ candidate: { strategyLineage: makeLineage(), strategyDecision: decision, pair: decision.pair, instrumentSpecSnapshotId: 'instrument-1' }, entryStopProposal: null,
...
describe('Phase 13 current valuation and strategy-instance ownership', () => {
  ...
  it.each(['0', '-1'])('rejects present %s instance quantity as unreconciled', (currentQuantity) => {
    const pair = openPair({ ownership: { status: 'RECONCILED', positionState: 'OPEN', accountId: 'account-1', pair: 'B-BTC_USDT', positionId: 'position-1',
      instanceOwnership: [{ strategyInstanceId: TEST_INSTANCE_ID, strategyId: 'EMA_TREND', strategyVersion: '1.0.0', parameterHash: TEST_PARAMETER_HASH, currentQuantity, currentNotionalInr: '80' }] } });
  ...
  it('rejects a mixed-sign set before cancellation can reconcile', () => {
    const pair = openPair({ ownership: { status: 'RECONCILED', positionState: 'OPEN', accountId: 'account-1', pair: 'B-BTC_USDT', positionId: 'position-1',
      instanceOwnership: [
        { strategyInstanceId: TEST_INSTANCE_ID, strategyId: 'EMA_TREND', strategyVersion: '1.0.0', parameterHash: TEST_PARAMETER_HASH, currentQuantity: '11', currentNotionalInr: '88' },
  ...
  it('does not authorize a same-strategy sibling instance', () => {
    const pair = openPair({ ownership: { status: 'RECONCILED', positionState: 'OPEN', accountId: 'account-1', pair: 'B-BTC_USDT', positionId: 'position-1',
      instanceOwnership: [{ strategyInstanceId: 'instance-2', strategyId: 'EMA_TREND', strategyVersion: '1.0.0', parameterHash: TEST_PARAMETER_HASH, currentQuantity: '10', currentNotionalInr: '80' }] } });
    const result = new RiskEngine(makePolicy()).evaluateRisk(closeContext(pair));
  ...
  it.each(['strategyId', 'strategyVersion', 'parameterHash'] as const)('rejects a %s mismatch on the matching instance identity', (field) => {
    const record = { strategyInstanceId: TEST_INSTANCE_ID, strategyId: 'EMA_TREND', strategyVersion: '1.0.0', parameterHash: TEST_PARAMETER_HASH, currentQuantity: '10', currentNotionalInr: '80', [field]: 'wrong' };
  ...
  it('responds exactly to mark-price and conversion-rate changes', () => {
    const priced = openPair({ position: { state: 'OPEN', positionId: 'position-1', positionDirection: 'LONG', quantityMagnitude: '10', valuation: valuation({ valuationPriceUsdt: '101', unitValuationInrPerQty: '8.08', aggregateCurrentNotionalInr: '80.8' }) },
      ownership: { status: 'RECONCILED', positionState: 'OPEN', accountId: 'account-1', pair: 'B-BTC_USDT', positionId: 'position-1', instanceOwnership: [{ strategyInstanceId: TEST_INSTANCE_ID, strategyId: 'EMA_TREND', strategyVersion: '1.0.0', parameterHash: TEST_PARAMETER_HASH, currentQuantity: '10', currentNotionalInr: '80.8' }] } });
    ...
    const converted = openPair({ position: { state: 'OPEN', positionId: 'position-1', positionDirection: 'LONG', quantityMagnitude: '10', valuation: valuation({ conversionRateInrPerUsdt: '81', unitValuationInrPerQty: '8.1', aggregateCurrentNotionalInr: '81' }) },
      ownership: { status: 'RECONCILED', positionState: 'OPEN', accountId: 'account-1', pair: 'B-BTC_USDT', positionId: 'position-1', instanceOwnership: [{ strategyInstanceId: TEST_INSTANCE_ID, strategyId: 'EMA_TREND', strategyVersion: '1.0.0', parameterHash: TEST_PARAMETER_HASH, currentQuantity: '10', currentNotionalInr: '81' }] } });
  ...
  it('reconciles huge canonical Decimal values exactly inside the context', () => {
    ...
    const hugePair = openPair({ contractMultiplier: '1', position: { state: 'OPEN', positionId: 'position-1', positionDirection: 'LONG', quantityMagnitude: quantity,
      ...
      ownership: { status: 'RECONCILED', positionState: 'OPEN', accountId: 'account-1', pair: 'B-BTC_USDT', positionId: 'position-1', instanceOwnership: [{ strategyInstanceId: TEST_INSTANCE_ID, strategyId: 'EMA_TREND', strategyVersion: '1.0.0', parameterHash: TEST_PARAMETER_HASH, currentQuantity: quantity, currentNotionalInr: notional }] } });
  ...
  it('returns numeric-context exhaustion as a RejectedRiskDecision', () => {
    ...
    const pair = openPair({ contractMultiplier: huge, position: { state: 'OPEN', positionId: 'position-1', positionDirection: 'LONG', quantityMagnitude: '1',
      ...
      ownership: { status: 'RECONCILED', positionState: 'OPEN', accountId: 'account-1', pair: 'B-BTC_USDT', positionId: 'position-1', instanceOwnership: [{ strategyInstanceId: TEST_INSTANCE_ID, strategyId: 'EMA_TREND', strategyVersion: '1.0.0', parameterHash: TEST_PARAMETER_HASH, currentQuantity: '1', currentNotionalInr: '1' }] } });
    const settlement = seal({ ...makeSettlement(), rateInrPerUsdt: '1' });
