...
export class PrismaCandle1mRepository implements Candle1mRepository, Canonical1mRangeReader {
  ...
  #mapRowToCanonicalCandle(row: {
    ...
    return createCanonicalCandle1m({
      ...
      // No decimal-place argument: preserve the full persisted value without rounding or exponent notation.
      open: CanonicalDecimal.from(row.open.toFixed()),
      high: CanonicalDecimal.from(row.high.toFixed()),
      low: CanonicalDecimal.from(row.low.toFixed()),
      close: CanonicalDecimal.from(row.close.toFixed()),
      volume: CanonicalDecimal.from(row.volume.toFixed()),
      quoteVolume: row.quoteVolume !== null ? CanonicalDecimal.from(row.quoteVolume.toFixed()) : null,
      source: row.source,
