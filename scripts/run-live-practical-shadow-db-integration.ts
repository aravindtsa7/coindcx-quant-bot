/**
 * Phase 18B Checkpoint C strict real-database acceptance runner.
 *
 * Ordinary `npm test` treats
 * `tests/integration/execution/live-practical-shadow.integration.test.ts`
 * as soft-skippable (the repository's live-DB integration convention). This
 * script sets `REQUIRE_LIVE_PRACTICAL_SHADOW_DB_INTEGRATION=1`, under which
...
const result = spawnSync(
  ...
  {
    stdio: 'inherit',
    env: { ...process.env, REQUIRE_LIVE_PRACTICAL_SHADOW_DB_INTEGRATION: '1' },
  },
);

if (result.error) {
  console.error('[P18B-SHADOW-DB] failed to launch vitest:', result.error);
  process.exit(1);
}
