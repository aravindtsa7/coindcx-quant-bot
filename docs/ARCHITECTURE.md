...
- **One Rejection Taxonomy, One Global Precedence Table:** A `decisionId` (or any other) recomputation mismatch is always `DECISION_IDENTITY_MISMATCH` (a rejected decision), never `RISK_SOURCE_INVALID` (a thrown config/programming error) — `RISK_SOURCE_INVALID` is reserved strictly for input so malformed that canonical identity cannot be established at all. A frozen 13-step evaluation pipeline determines *when* each check runs, but final reason ordering — including multiple reasons discovered within a single step — is decided exclusively by index into one canonical, self-validating `REJECTION_PRECEDENCE_V1` table (every known rejection code exactly once), never by incidental object/array/Map/Set iteration order.
...
- **Deterministic Identity & Replay:** `riskDecisionId`/`positionSizingDecisionId` are computed via the repository's single canonical-JSON SHA-256 hasher (`sha256CanonicalJson`, reused from Phase 9/11/12) over frozen policy identities and independently-computed content hashes of every snapshot, with canonical Decimal normalization (`"1"`/`"1.0"`/`"1.000"` hash identically) and exact-shape ingestion (no unknown field silently enters or escapes identity) — strictly excluding wall-clock reads, host/PID/worker identifiers, and mutable object ordering.
