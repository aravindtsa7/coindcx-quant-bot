...
| **Phase 7** | **Historical Dataset Engine** | Deterministic, reproducible historical canonical 1m dataset engine: resumable chunked REST backfill via CoinDcxFuturesCandleRestReader, zero synthetic gap-filling, immutable MySQL candles_1m persistence, deterministic SHA-256 content/datasetId hashing, manifest tracking, streaming NDJSON export/import, and Phase 6 higher-timeframe parity. | CURRENT |
