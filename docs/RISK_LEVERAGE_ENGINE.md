# Risk / Leverage Engine — Phase 13 Architecture & Specification (Corrected, Round 6)
...
- **Aggregate notional reconciliation (`[P13-B03 R5]` — now fully deterministic):** the prior revision delegated this to "the adapter's own reconciliation rules," which froze nothing. Replaced by an exact, engine-side recomputation against the canonical valuation basis (§3.1.1), in this fixed order:
  ...
  2. **Valuation inputs must be bound, not asserted.** `valuation.contractMultiplier` must equal `PairRiskSnapshot.contractMultiplier`, `valuation.conversionRateInrPerUsdt` must equal `SettlementConversionSnapshot.rateInrPerUsdt`, and `valuation.valuationMethodVersion` must equal `RiskValuationPolicy.valuationMethodVersion` (§10.5) — each exactly, on canonical normalized decimals/strings. Any mismatch is evidence self-inconsistency: `RISK_SOURCE_INVALID` (§13.2). A `conversionMarket` mismatch against the settlement snapshot's market identity is `SETTLEMENT_RATE_UNAVAILABLE` (§8.4, unchanged rule); a `valuationPriceSourceId`/`conversionSourceId` mismatch is `SOURCE_ID_MISMATCH` (§8.6, unchanged rule). No new rejection code is introduced.
  ...
  Failing **any** of the above — quantity reconciliation or notional reconciliation — means the evidence does not prove what a `RECONCILED` claim asserts. **Phase 13 treats this exactly as `UNRECONCILED` for every gating purpose** (`POSITION_OWNERSHIP_UNRECONCILED`, Group B) rather than trusting a partially-consistent `RECONCILED` claim — there is no intermediate "trusted enough" state, and a `RECONCILED` claim is never accepted on quantity alone while its notionals disagree.
...
Any of the following makes the ownership state `UNRECONCILED` (`POSITION_OWNERSHIP_UNRECONCILED`, Group B — no new code): a record with `currentQuantity <= 0`; a record whose `currentQuantity` or `currentNotionalInr` is non-canonical, non-finite, or otherwise unparseable as a `RiskCalcDecimal`; a set whose magnitudes do not sum exactly to `position.quantityMagnitude`; or an adapter supplying signed instance quantities or a per-instance direction at all. The validation order is: per-record sign/canonicality first, then the aggregate sums (§2.8.1) — so a mixed-sign set is rejected on its own terms and never reaches a summation that might mask it.
...
**Ingestion validation (§11.2 exact-shape, extended):** a `PairPositionState.OPEN` whose `quantityMagnitude` is negative, zero, non-finite, or non-canonical is malformed evidence — the adapter failed to apply this boundary — and fails closed with `PAIR_STATE_UNAVAILABLE` (Group B, the existing malformed-instrument-snapshot code, §3.2). No new rejection code is introduced.
...
This is the single frozen source of truth for how sizing, action, and the accepted/rejected envelope relate — `NOT_APPLICABLE` (§7.2/§7.5) replaces what was previously an inconsistent implicit null/non-null relationship for `CLOSE`/`NO_CHANGE`/`REVERSAL_DEFERRED`.
...
| Condition | Rejection |
|---|---|
| `entryPriceUsdt ≤ 0`, non-finite, `< minPrice`, `> maxPrice`, or not tick-aligned | `INVALID_ENTRY_PRICE` |
...
This step only executes for `action = 'OPEN'` (§2.5–§2.7 exempt every other action).
...
```
`RiskDecision.approved` (when `ACCEPTED`) is required to be **field-for-field identical** to `PositionSizingDecision.sizing` for `action = 'OPEN'` — `RiskDecision` never recomputes or diverges from the sizing numbers; it only adds exposure/loss/drawdown gates on top (§9–§10) and the final envelope. For `action = 'CLOSE'`, `PositionSizingDecision.outcome` is always `'NOT_APPLICABLE'` and `RiskDecision`'s CLOSE-shaped `approved` (§5.2) is computed directly from `PairRiskSnapshot.ownership` (§2.8), not from `PositionSizingDecision.sizing`. See §5.4 for the complete action-by-action relationship table.
...
| Condition | Rejection |
|---|---|
| `evaluationTimeMs - observedAtMs > policy.maxXSnapshotAgeMs` | `*_STATE_STALE` / `LEVERAGE_TIERS_STALE` / `SETTLEMENT_RATE_STALE` (per snapshot) |
...
| `valuation.contractMultiplier` ≠ `PairRiskSnapshot.contractMultiplier`, `valuation.conversionRateInrPerUsdt` ≠ `SettlementConversionSnapshot.rateInrPerUsdt`, or `valuation.valuationMethodVersion` ≠ `RiskValuationPolicy.valuationMethodVersion` | `RISK_SOURCE_INVALID` (§13.2, §2.8.1 step 2) — the valuation is bound to inputs the rest of the evidence contradicts |
...
| any snapshot's `provenance.sourceId` ≠ its policy-bound expected source ID (§8.6) | `SOURCE_ID_MISMATCH` (Group B) |
...
Every canonical contract type (every snapshot, every config, every proposal) is validated at ingestion against its **exact** expected key set — `Object.keys(value).length === expectedKeys.length` and every expected key present — mirroring the existing pattern in `src/strategies/core/identity.ts:29-32`. Unknown or extra fields are rejected at ingestion (`RISK_SOURCE_INVALID`, Group C for config objects; a per-trade input's extraneous field is likewise a construction fault since it indicates the bridge itself is non-conformant, not a business condition) — they can neither silently enter nor silently escape the canonical identity.
...
- **Static/construction-time (Group C, throws, no decision constructed):** malformed `GlobalRiskConfig`/`PairRiskConfig`/`RiskModeConfig`/`RiskOverride` (hierarchy violation, widening override, non-finite config decimal, wrong exact shape), unsupported risk mode, `expectedRiskPolicyId` mismatch — and, per §11.2's exact-shape ingestion, an incoming `StrategyDecision`/proposal/snapshot object carrying an unknown or missing required field, or a field of the wrong type (non-safe-integer timestamp, non-string identity field, a `status`/`targetExposure` combination the genuine Phase 10 kernel could never produce, §2.3). These are evaluated **once**, before any decisionId recomputation is even attempted — canonical source identity itself cannot be established from malformed shape, so there is nothing to attach a `riskDecisionId` to.
- **Per-evaluation (Group A/B, always produces a `RejectedRiskDecision`):** everything else, **without exception**, once the incoming `StrategyDecision`/proposals/snapshots have passed exact-shape validation above. This explicitly includes a `decisionId` **recomputation mismatch** — the object is well-formed enough to compute a candidate hash from, it simply does not match what was asserted, which is exactly the situation `DECISION_IDENTITY_MISMATCH` exists to describe.

**The exact, singular disambiguating rule (frozen, replacing the two-different-answers version):** `RISK_SOURCE_INVALID` is restricted to cases where canonical source identity **cannot be established at all** — a structurally malformed object, an unknown field, a field of the wrong type. Once an object is well-formed enough that Phase 13 *can* compute a canonical hash or identity from it, any mismatch between that computed value and an asserted one is **always** `DECISION_IDENTITY_MISMATCH` (Group B), **never** `RISK_SOURCE_INVALID` (Group C) — regardless of which field the mismatch involves (`decisionId`, cross-field pair identity, account identity uses its own `ACCOUNT_IDENTITY_MISMATCH`, position identity uses `POSITION_IDENTITY_MISMATCH`, §8.4). There is now exactly one behavior per condition, not two.
...
2. **Canonical source shape** — exact-shape validation (§11.2) of the incoming `StrategyDecision`, `EntryStopProposal`, `LeverageProposal`, and every snapshot. A violation throws `RISK_SOURCE_INVALID` (Group C) — canonical identity cannot be established (§13.2).
...
6. **Snapshot identity/provenance** — every snapshot's `contentSha256` recomputed by Phase 13 itself (§11.5) and cross-checked; never trusted from a caller-supplied hash; every snapshot's `provenance.sourceId` compared against its policy-bound expected value (§8.6, `[P13-M05 R3]`). Mismatch ⇒ `SOURCE_ID_MISMATCH` (Group B).
7. **Temporal/freshness** — strict causal ordering (§8.4, zero tolerance) and policy-owned max-age staleness (§8.3) for every snapshot required by the not-yet-derived action. Violations ⇒ `EVIDENCE_TEMPORAL_CAUSALITY_VIOLATION` / `*_STATE_STALE` family (Group B).
...
- Strict causal ordering (§8.4) and policy-owned freshness (§8.3) — a stale or future-dated conversion is rejected identically to any other stale/future evidence.
...
`[P13-M08 R2]` `RISK_SOURCE_INVALID` is strictly restricted to canonical-source-shape failures (§13.2, §13.3 step 2) — it is never used for a `decisionId` (or any other) recomputation *mismatch*, which is always `DECISION_IDENTITY_MISMATCH` (Group B) regardless of which field is involved. `[P13-M08 R3]` `RISK_CONFIG_INVALID` additionally covers a malformed `REJECTION_PRECEDENCE_V1` (a duplicate entry, or a missing Group A/B code, §13.4) and an invalid `maxDailyLossInr`/`globalMaxDailyLossInr` (§10.4).
...
src/risk/
  ...
  config.ts            — Global/Pair/RiskMode/Override/RiskSourceAuthorityPolicy/RiskValuationPolicy normalization (§10.4–§10.5),
                          ...
                          riskModeConfigId / riskSourceAuthorityPolicyId / riskPolicyId (§11.3–§11.4)
...
5. **`PositionSizingDecision` identity and outcome discrimination:** `positionSizingDecisionId` changes iff any field of `PositionSizingDecision` changes; `outcome` is `'NOT_APPLICABLE'` for every `CLOSE`/`NO_CHANGE`/`REVERSAL_DEFERRED` candidate (never `'NOT_SIZED'`, which is reserved for a failed `OPEN`); `RiskDecision.approved` is field-for-field identical to `PositionSizingDecision.sizing` on every `AcceptedOpenRiskDecision`; `sourcePositionSizingDecisionId` is never `null` on any `RiskDecision`, including rejections.
...
8b. **Canonical current-notional valuation & aggregate-notional reconciliation (`[P13-B03 R5]`, §3.1.1/§2.8.1):**
   ...
   - **Conversion identity must match the settlement snapshot:** `valuation.conversionRateInrPerUsdt` ≠ `SettlementConversionSnapshot.rateInrPerUsdt` rejects with `RISK_SOURCE_INVALID`; a `conversionMarket` naming a different market rejects with `SETTLEMENT_RATE_UNAVAILABLE`; a `conversionSourceId`/`valuationPriceSourceId` outside `RiskSourceAuthorityPolicy` rejects with `SOURCE_ID_MISMATCH`. A valuation built from `settlementCurrencyAvgPriceInrPerUsdt` (the position's historical average, §3.1.1) can never satisfy this check.
...
18. **Settlement conversion market identity (`[P13-E03]`):** correct `conversionMarketId` + correct currencies accepts; correct currencies but wrong `conversionMarketId` rejects (`SETTLEMENT_RATE_UNAVAILABLE`) even though currencies alone "look right"; reversed currencies (`INR`→`USDT`) rejects; zero/negative rate rejects; stale conversion rejects (`SETTLEMENT_RATE_STALE`); future-dated conversion rejects (`EVIDENCE_TEMPORAL_CAUSALITY_VIOLATION`); a tampered `contentSha256` that doesn't match Phase 13's own recomputation rejects; a correct conversion changes `notionalInr`/`estimatedInitialMarginInr` predictably.
19. **Canonical identity mechanics:** `"1"`/`"1.0"`/`"1.000"` canonicalize identically and do not change any hash; an unknown extra field on any canonical input rejects at ingestion (`RISK_SOURCE_INVALID`); object key-order permutation never changes a hash.
...
21. **Total reason precedence — multiple simultaneous failures within one pipeline step (`[P13-M08 R3]`, §13.4):**
    ...
    - **Multiple snapshot source/hash failures at once** (e.g. a wrong `sourceId` on two different snapshots, step 6) — ordering matches `REJECTION_PRECEDENCE_V1`, not discovery order.
...
**P13_B03_SIGN_CORRECTION_READY_FOR_FINAL_VERIFY**
